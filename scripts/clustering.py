...
def main(input_matrix, input_matrix_ids, meta_path, outpath):
    ...
    clusters = pd.DataFrame({'indiv_id': mat.index, 'genotype_cluster' : cl}).sort_values(by = 'genotype_cluster')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--matrix-ids', type = str, help = 'Result of plink2 --king with suffix .king.id')
    parser.add_argument('--meta-file',  type = str, help = 'Path to meta file')
    parser.add_argument('--outpath',    type = str, help = 'Path to directory to save updated metafile and visualizations')
