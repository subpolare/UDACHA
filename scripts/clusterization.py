...
def main():
    ...
    n_k = K.shape[0]
    print(f'✔️ [1/5] Loaded KING matrix with shape {K.shape}')
    ...
    print(f'✔️ [2/5] Loaded {len(sample_id)} sample IDs')

    np.fill_diagonal(K, 0.0)
    K[K < args.dist0] = 0.0
    print(f'✔️ [3/5] Zeroed KING distances < {args.dist0}')

    corr_dist = pdist(K, metric = 'correlation')
    Z = linkage(corr_dist, method = 'complete')
    labels = fcluster(Z, t = args.cut, criterion = 'distance')
    print(f'✔️ [4/5] Performed hierarchical clustering with cut {args.cut}')

    out = pd.DataFrame({'sample_id' : sample_id, 'geno_cluster_id' : labels.astype(int)})
    out.to_csv(args.output, sep = '\t', index = False)
    print(f'✔️ [5/5] Saved clusters to {args.output}')
